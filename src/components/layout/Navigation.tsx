...
export default function Navigation(props: NavbarProps) {
  ...
  // ✅ FIXED: Navigation items with BOTH text and href editableIds
  const navigation = [
  {
    name: "Home",
    href: "#hero",
  ...
  },
  {
    name: "About",
    href: "#about",
    textEditableId: "text-nav-about",
    hrefEditableId: "link-nav-about"
  },
  {
    name: "Contact",
    href: "#contact",
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center space-x-2">
              ...
              <span className={cn(
                ...
                config.isTransparent && !isScrolled && config.lightText ? "text-white" : "text-foreground"
              )}>
              ...
              {navigation.map((item) => (
                <Link
                  ...
                  className={cn(
                    ...
                    (!config.isTransparent || isScrolled) && "text-muted-foreground hover:text-accent-foreground hover:bg-accent",
                    // TRANSPARENT MODE: Light text only when transparent and not scrolled
                    config.isTransparent && !isScrolled && config.lightText && "text-white/90 hover:text-white hover:bg-white/10",
                    config.isTransparent && !isScrolled && !config.lightText && "text-muted-foreground hover:text-accent-foreground hover:bg-accent/50"
                  )}
                >
          ...
          <div className="hidden md:flex items-center space-x-4">
            <Button 
              ...
              className={cn(
                config.isTransparent && !isScrolled && config.lightText && "text-white hover:text-white hover:bg-white/10"
              )}
            ...
            <Button 
              ...
              className={cn(
                "group",
                config.isTransparent && !isScrolled && config.lightText && "bg-white text-black hover:bg-white/90"
              )} 
